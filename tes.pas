...
begin
    while(i <= jmlhPemain - 2) do
        begin
            if(selisihSkor[i] = selisihSkor[i+1]) then
                begin
                    pemainTieBreaker[jumlahPemainTieBreaker] := namaPemain[i];
                    pemainTieBreaker[jumlahPemainTieBreaker+1] := namaPemain[i+1];
                    selisihSkorPesertaTieBreak[jumlahPemainTieBreaker] := selisihSkor[i];
                    selisihSkorPesertaTieBreak[jumlahPemainTieBreaker+1] := selisihSkor[i+1];
                    jumlahPemainTieBreaker += 2;
                    i += 2; 
                end
            else if(i > 1) then
                begin
